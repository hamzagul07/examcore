...
type DeepLink = { taskId: string | null; why: boolean }
 ...
 * task's check-in once it shows as done) and ?why=1 (the dashboard card's
 * "Why this?" link). All three are stripped so a reload does not count or
 * open them twice.
 */
export function StudyPlanScreen({ initial, evidence, firstName, subjectOptions, profile }: Props) {
  ...
  const [deepLink, setDeepLink] = useState<DeepLink>({ taskId: null, why: false })
  ...
  useEffect(() => {
    try {
      ...
      if (task) setDeepLink({ taskId: task, why })
      if (src !== null || task !== null || sp.has('why')) {
        sp.delete('src')
        sp.delete('task')
        sp.delete('why')
        const q = sp.toString()
  ...
  return (
    <RoadmapSetup
      ...
      onBuilt={(next: SetupSaved) => {
        ...
        setDeepLink({ taskId: null, why: false })
        setView('screen')
        window.scrollTo({ top: 0 })
      }}
