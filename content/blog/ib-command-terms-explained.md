...
Higher-order terms (`analyse`, `evaluate`, `discuss`) require structure: claim → evidence → link → (for evaluate) judgement. Lower-order terms (`define`, `outline`) need precision and brevity.
...
## Paper-specific tips
**Compare** = similarities and differences with explicit linking. **Contrast** = differences only. **To what extent** = thesis with counterargument. **Discuss** = balanced exploration, not one-sided rant.
