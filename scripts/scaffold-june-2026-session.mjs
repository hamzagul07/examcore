...
function scaffold(code) {
  ...
  if (!fs.existsSync(srcFile)) {
    console.error(`No data file for ${code} ÿ create content/data/grade-boundaries/${code}.json first.`)
    return false
  }
  ...
  console.log(`  wrote ${path.relative(ROOT, out)} (${session.components.length} components ÿ verify thresholds from PDF)`)
  return true
}
