...
1. **Thick, muscular wall** withstands the high pressure of blood leaving the heart.
2. **Elastic fibres** stretch as blood surges and recoil to maintain pressure between beats.
3. **Narrow lumen** helps keep blood pressure high.
4. **No valves** (except at the heart) because high pressure prevents backflow.
...
The command term controls **what counts** as a marking point. The same biology, marked under two different verbs, earns marks in completely different ways. Misreading the verb is one of the most common ways strong candidates lose marks they knew the content for.
...
The verb is an instruction, not decoration. *"Describe the events of the cardiac cycle"* wants the sequence; *"Explain why it is myogenic"* wants the cause. And *"Compare"* is the classic trap: two separate paragraphs, one per organism, earn nothing — comparison marks need statements that hold both items in the same sentence. Drill the verbs that catch you in the [full IB command terms explainer](/blog/ib-command-terms-explained).

## Writing mechanisms, not vague statements

Examiners tick **specific reasons**, never hand-waves. A sentence that could apply to almost any topic in the syllabus is filler and scores zero — even when it is technically true.
...
Data questions reward a specific three-move routine, and students who skip straight to explanation leave easy marks behind.

1. **Read values with units off the graph or table.** Quote the actual figure — "the rate rises from 2 to 8 mmol s⁻¹" earns a mark that "it goes up" does not. Units are part of the point; dropping them can cost you.
2. **Describe the trend precisely, including anomalies.** State the overall pattern *and* flag where the data breaks it: "the rate increases steadily to a peak at 40 °C, then falls sharply" — and if one point sits off the line, name it as an anomaly. Vague trend statements ("it changes") score nothing.
3. **Use the data to support a conclusion.** When a question asks what the data *shows* or *suggests*, refer back to specific values as evidence rather than answering from general knowledge: "the higher yield at pH 7 suggests the enzyme's optimum is near neutral."
...
The long questions (often worth [7] or [8], sometimes with a quality mark) are still marked point-by-point — they just need **more** of everything. Three things win them:
...
Some question types carry marks for **working shown**, not just the final answer:
...
Paper 2 is a marking checklist, not an essay. Read the tariff and write one distinct point per mark; match the command term exactly — mechanisms for "explain", linked statements for "compare"; name processes instead of hand-waving; quote data with units and describe trends before explaining them; and show full working on genetics and diagrams. Practise to the tariff, [get your answers marked](/mark), and revisit the [how to get a 7 in IB Biology](/blog/ib-biology-how-to-get-a-7) guide to fit this technique into a wider study routine. Do that consistently and the marks stop being a matter of luck.
