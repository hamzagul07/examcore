...
async function call(sys, user, maxTok = 8000) {
  ...
  for (let a = 1; a <= 4; a++) {
    const res = await fetch(url, { method: 'POST', headers: { authorization: `Bearer ${await token()}`, 'content-type': 'application/json' }, body: JSON.stringify({ systemInstruction: { parts: [{ text: sys }] }, contents: [{ role: 'user', parts: [{ text: user }] }], generationConfig: { temperature: 0.2, maxOutputTokens: maxTok, responseMimeType: 'application/json' } }) })
...
let fixed = 0, kept = 0, skipped = 0
for (const v of high) {
  const locs = byId.get(v.id)
  if (!locs) { console.log(`  ? ${v.id}: not found`); continue }
  try {
    const { file, i } = locs[0]
    ...
    // Re-verify the correction independently
    ...
    // Apply to ALL locations sharing this id (hl+sl)
