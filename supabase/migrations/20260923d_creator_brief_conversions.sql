-- Creators: the weekly brief and the conversions ledger (docs/CREATORS_PROGRAM.md).
--
-- brief_*: the Monday email. Dedup stamp plus an opt-out that the one-click
-- unsubscribe link flips (kind 'creator' in lib/community/email-unsubscribe.ts).
--
-- creator_conversions: one row per (subscription, event) for an account whose
-- signup was attributed to a creator. Written by the Polar webhook. Nothing
-- pays out from it yet; it exists so that revenue share, when it comes, can
-- be paid on a record kept from the first day.
...
create table if not exists public.creator_conversions (
  ...
  unique (polar_subscription_id, event)
);

alter table public.creator_conversions enable row level security;
revoke all on table public.creator_conversions from public, anon, authenticated;

create index if not exists creator_conversions_creator_idx
  on public.creator_conversions (creator_id, created_at desc);

-- Register the ledger as service-only. Body copied from the latest
-- definition (20260923_creators.sql) with only that addition.
