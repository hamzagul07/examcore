---
title: Cambridge 0620 Chemistry Grade Boundaries 2026
description: 0620 IGCSE Chemistry grade boundaries 2026 — IGCSE grades 18 August, threshold tables ~13 August. Estimate MCQ and theory papers with the calculator.
date: 2026-06-19
keywords: 0620 grade boundaries, 0620 grade boundaries 2026, Cambridge IGCSE Chemistry grade boundaries, 0620 chemistry thresholds, 0620 raw marks to grade
category: mark-schemes
author: hassan
updated: 2026-07-07
featured: true
informationGain: synthesis
---

Official **0620 Chemistry** grade boundaries for **June 2026** publish in August — **IGCSE grades 18 August 2026**, component **threshold tables around 13 August**. Estimate with marked past papers via the [0620 calculator](/tools/grade-boundary-calculator/0620).

## Quick answer

| Question | Answer |
|----------|--------|
| When are June 2026 **IGCSE grades** released? | **18 August 2026** (06:00 GMT) |
| When are **threshold tables** published? | **~13 August 2026** |
| Papers? | MCQ + theory + practical / alternative to practical (check your entry) |
| Marking style? | **Equations, moles, and precise terms** on structured Qs |
| Estimate tool | [0620 grade calculator](/tools/grade-boundary-calculator/0620) |

[Post-exam prep](/blog/cambridge-post-exam-results-prep-2026) — [Results day guide](/blog/cambridge-results-day-august-2026-guide)

## What are grade boundaries?

Grade boundaries are the minimum **raw marks** for each grade on every **0620** component — MCQ, theory, and practical or ATP. Cambridge sets a separate threshold per component code; your **overall** grade is a weighted aggregate where **A*** reflects syllabus performance, not a single paper in isolation. Percentages from classroom tests do not translate directly to IGCSE raw marks.

## How Cambridge sets 0620 boundaries

Examiners finish marking before thresholds are published. Harder stoichiometry or structured papers usually mean **lower** raw cut-offs for the same grade — comparable outcomes, not "grade inflation." Check your exact paper numbers in the PDF; using last year's **5070** O-Level table for **0620** is a common results-week mistake ([insights](/insights)).

## Core vs Extended at a glance

| Tier | Typical papers | Grade range |
|------|----------------|-------------|
| Core | Paper 1 + Paper 3 (+ practical route) | C to G |
| Extended | Paper 2 + Paper 4 (+ practical route) | A* to E |

Confirm your entry with your exams officer — mixing tiers is not allowed.

## How to estimate your grade

1. Timed paper for your tier — mark **state symbols, mole ratios, and units** against the scheme without leniency.  
2. Split lost marks into calculation slips vs recall gaps so revision targets the right skill.  
3. Compare component raw marks to June 2024/2023 in the [0620 calculator](/tools/grade-boundary-calculator/0620), using exact component codes from your entry form.

## Frequently asked questions

### When are official boundaries released?

Threshold PDFs **~13 August**; grades **18 August**.

### Is 80% always an A*?

No — raw thresholds change every session.

### IGCSE vs O-Level Chemistry?

Confirm your **0620** code on your statement of entry — papers differ from 5070.

## Related subjects

Each science code has its own threshold PDF — use these guides for revision links and calculator paths. Never copy threshold numbers between syllabuses, sessions, or paper variants. See the [grade boundaries hub](/guides/grade-boundaries) for every syllabus calculator. Cross-link below for adjacent syllabuses only; never swap threshold numbers between codes or sessions.

- [0610 Biology](/blog/cambridge-0610-biology-grade-boundaries-2026)
- [0625 Physics](/blog/cambridge-0625-physics-grade-boundaries-2026)
- [5070 O-Level Chemistry](/blog/cambridge-5070-chemistry-grade-boundaries-2026)

## Bottom line

**0620 grade boundaries 2026** land **18 August** (grades) with threshold PDFs **~13 August**. Marked past papers with strict equation marking, then the [0620 calculator](/tools/grade-boundary-calculator/0620), outperform any leaked spreadsheet — save component codes when Cambridge publishes the official row. Your exams officer confirms component codes on the statement of entry.