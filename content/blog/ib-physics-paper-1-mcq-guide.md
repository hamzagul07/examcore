...
Many avoidable Paper 1 errors happen before any physics does — in misreading the stem, where one word can flip the answer.
...
Multiple choice hands you the answer — it is among the four options, and your job is to remove the impostors. **Elimination is usually faster and safer than solving from scratch**, especially when you are unsure.
...
For questions that need a number, two fast checks catch most mistakes without redoing the calculation.
...
With roughly **one to one-and-a-half minutes per question**, Paper 1A rewards steady momentum over perfectionism. The trap is sinking five minutes into one stubborn question while easy marks wait untouched later.
...
Multiple choice is a trainable skill, and the fastest improver is **timed practice on real question styles** — working under the clock builds the pattern-recognition that spots a factor-of-two trap or a prefix slip on sight.
...
None of these require more physics — they are technique leaks that drain marks from candidates who already know the material.
...
Paper 1A rewards technique as much as knowledge. Because there is no negative marking, the first and last rule is to **answer every question** — eliminate what you can, guess what you must, and never leave a blank. Read qualifiers carefully, convert units, run quick unit and order-of-magnitude checks to dodge the factor-of-two and prefix traps, sanity-check with physical intuition, and keep moving with a flag-and-return rhythm. Drill real-style questions under the clock, review every miss by name, and multiple choice becomes one of the most reliable places on the exam to bank marks.
