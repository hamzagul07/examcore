...
/**
 * Shown atop a CAIE course lesson when opened from an AQA study path.
 * Keeps board identity + pushes visual learn → AQA mark.
 */
export function AqaLessonBridge({
  contentCode,
  label,
  markHref,
  subjectHubHref,
  ...
  return (
    ...
    >
      <div className="rounded-2xl border-2 border-[color-mix(in_srgb,var(--ec-brand)_35%,var(--ec-border))] bg-[color-mix(in_srgb,var(--ec-brand)_8%,var(--ec-surface-raised))] px-4 py-4 sm:px-5">
        <p className="ms-overline mb-1">AQA A-level · {contentCode}</p>
        <p className="text-sm font-semibold text-[var(--ec-text-primary)] sm:text-base">
          Mapped visual lesson — play the diagram, then mark with AQA conventions for{' '}
          {label}.
        </p>
