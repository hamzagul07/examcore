...
function buildPost(s) {
  ...
  const mistakeList = s.mistakes.map((m) => `- ${m}`).join('\n')
...
Cambridge **${s.name}** (${s.code}) is one of the most searched ${levelSlug} subjects for **past papers** and **mark scheme** help. This guide explains how the syllabus is examined, how marks are awarded, and how to turn practice into real marks — not just completed pages.

## Syllabus code ${s.code} at a glance
...
Past papers are labelled **${s.code}/XX** where **XX** is the component (paper) number for that session. Always check you are practising the right **component** for your route.
...
## Common mistakes students make on ${s.code}
...
Not every question comes from a past paper. On [MarkScheme](/mark), choose **My question**, select **${s.name} (${s.code})**, add the **question** (photo or text), upload your **answer**, and get Cambridge-style feedback (method marks, bands, or point marks depending on the question type).

## Using MarkScheme for ${s.code} past papers

For real past-paper questions, use **Past paper** mode so we can match the official mark scheme when it is in our library. Upload clear photos of your handwriting — see our guide on [photographing handwritten answers](/blog/photograph-handwritten-past-paper-answers).
...
${s.code} rewards **precision against the mark scheme**, not vague knowledge. Learn the language examiners use, mark honestly, and fix one repeatable error at a time — that is how ${levelSlug} ${s.name} scores move.
`
}
...
console.log('done —', SUBJECTS.length, 'subjects')
