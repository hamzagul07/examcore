...
- Closed for the existing route by `lib/teacher/override.ts` (since replaced by `lib/teacher/override-validate.ts`, below): total clamped to `0 ≤ total ≤ attempts.total_marks`, entries validated and stripped of raw HTML, reasoning length-capped, the AI original snapshotted once and kept.
...
- Closed by requiring sign-in for the preview and a persisted per-address guard on lookups and joins, failing closed with a 503 when the guard cannot be read. The interim guard (`lib/teacher/join-attempts.ts`) is deleted; `lib/rate-limit.ts` `consumeInviteLookupSlot` replaces it.
