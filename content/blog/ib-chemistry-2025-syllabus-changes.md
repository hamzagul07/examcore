...
The most fundamental change is philosophical. Instead of marching through numbered topics, the syllabus now hangs everything on **two organising concepts**, each split into three strands:
...
On the previous syllabus your class picked an optional topic from a menu, studied it in addition to the core, and answered on it in a dedicated paper. That is **gone**. The 2025 syllabus is a **single common body of content** that every candidate studies — no options, no menu, no separate option paper.
...
Assessment now runs on **two written papers** instead of three:
...
It is easy to over-panic about a syllabus change. Plenty is unchanged:
...
- **SL and HL still share a common core**, with **additional higher level (AHL)** content layered on for HL. The extra HL depth (entropy and Gibbs free energy, more demanding equilibrium and organic work) sits on top of the same concepts. If you are still deciding, read [IB Chemistry SL vs HL](/blog/ib-chemistry-sl-vs-hl).
...
The single biggest mindset shift: **revise connected concepts, not isolated topics.** A few concrete habits follow from the concept-based design:
...
A simple weekly loop beats cramming:

1. **Learn one concept strand at a time** — take S2 or R2 as a unit and understand it before drilling questions.
2. **Practise timed, mixed questions** that force you to connect strands, with the data booklet open so navigating it becomes automatic.
3. **Mark honestly and log dropped marks** by type — units, sig figs, method, vague explanation — then attack your recurring errors before the next session.

Start your scientific investigation IA early rather than in the final term, and use the [IB guides hub](/guides/ib) to structure the run-in. For paper-by-paper practice, the [IB Chemistry SL past papers guide](/blog/ib-chemistry-sl-past-papers-guide) shows how to use them well.
...
Yes. SL and HL share a **common core**, with **additional higher level (AHL)** content for HL that adds depth (entropy and Gibbs free energy, more demanding equilibrium and organic work). See [IB Chemistry SL vs HL](/blog/ib-chemistry-sl-vs-hl) if you are choosing.

## Bottom line

The 2025 IB Chemistry syllabus (first exams May 2025) is a genuine redesign in shape, not in substance: content is now organised around the **two concepts of Structure and Reactivity**, the **options are gone**, and assessment runs on **two papers** instead of three. The IA, the SL/HL core-plus-AHL split, and the underlying chemistry all carry over. Revise the connections between concepts rather than memorising isolated topics, verify the exact paper specifics with the official guide or your teacher, and use current, concept-mapped resources — and the change becomes an advantage rather than a hurdle.
