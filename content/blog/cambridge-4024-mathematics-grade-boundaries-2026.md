---
title: Cambridge 4024 Mathematics Grade Boundaries 2026 — June Thresholds Explained
description: 4024 O-Level Mathematics grade boundaries 2026 — O-Level grades 18 August, threshold tables ~13 August. Estimate both papers with the calculator.
date: 2026-06-19
keywords: 4024 grade boundaries, 4024 grade boundaries 2026, Cambridge Mathematics grade boundaries, 4024 mathematics thresholds, 4024 raw marks to grade
category: mark-schemes
author: hassan
updated: 2026-07-06
featured: true
informationGain: synthesis
---

Official **4024 Mathematics** grade boundaries for **June 2026** publish in August — **O-Level grades 18 August 2026**, component **threshold tables around 13 August**. Estimate with marked past papers via the [4024 calculator](/tools/grade-boundary-calculator/4024).

## Quick answer

| Question | Answer |
|----------|--------|
| When are June 2026 **O-Level grades** released? | **18 August 2026** (06:00 GMT) |
| When are **threshold tables** published? | **~13 August 2026** |
| Papers? | Non-calculator + calculator (both count to total) |
| Marking style? | **Method marks** — show working for M marks |
| Estimate tool | [4024 grade calculator](/tools/grade-boundary-calculator/4024) |

[Post-exam prep](/blog/cambridge-post-exam-results-prep-2026) · [Results day guide](/blog/cambridge-results-day-august-2026-guide)

## What are grade boundaries?

Minimum **raw marks** per grade — not fixed percentages. Your **overall** grade uses the sum of both papers.

## How Cambridge sets 4024 boundaries

Comparable outcomes after marking — harder papers usually mean lower thresholds.

## How to estimate your grade

1. Full timed paper (both components when ready)  
2. Mark strictly — **working for every M mark**  
3. [4024 calculator](/tools/grade-boundary-calculator/4024) with recent sessions  

Revision: [4024 past papers](/blog/cambridge-4024-o-level-mathematics-past-papers-guide) · [0580/9709 maths most repeated](/blog/most-repeated-cambridge-maths-past-paper-topics-2026) · [0580 IGCSE boundaries](/blog/cambridge-0580-mathematics-grade-boundaries-2026) · [mark a paper](/mark)

## Frequently asked questions

### When are official June 2026 boundaries released?

Threshold PDFs **~13 August**; final grades on your statement **18 August**.

### Can one weak paper be offset?

Yes — fight for marks on **both** papers; totals determine the grade.

### Is 80% always an A?

No — raw thresholds change every session.

## Related subjects

- [4037 Additional Maths](/blog/cambridge-4037-additional-mathematics-grade-boundaries-2026)
- [9709 A-Level Maths](/blog/cambridge-9709-mathematics-grade-boundaries-2026)
- [2281 O-Level Economics](/blog/cambridge-2281-economics-grade-boundaries-2026)

## Bottom line

**4024 grade boundaries 2026** land in August — **shown method on both papers** beats rumour spreadsheets.
